...
static char *load_text(char const* const filename )
{
  ...
  // no help file then sarch [pakset]/text([lang]
  if (!file) {
    //env_t::pak_name +
    file = dr_fopen((env_t::pak_name + PATH_SEPARATOR + file_prefix + translator::get_lang()->iso_base + PATH_SEPARATOR + filename).c_str(), "rb");
  }
